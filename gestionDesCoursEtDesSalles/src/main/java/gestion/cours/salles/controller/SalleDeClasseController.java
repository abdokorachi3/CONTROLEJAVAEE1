...
public class SalleDeClasseController {
    ...
    public String ajoutsalles(SalleDeClasse salleDeClasse) {
        return"add-salles";
    }

    @PostMapping("/addsalles")
    public String addsalles(@Validated SalleDeClasse salleDeClasse, BindingResult result, Model model) {
        if (result.hasErrors()) {
            return"add-salles";
        }
    ...
    @GetMapping("/salles/afficherplus/{id}")
    public String afficherplus(@PathVariable("id") long id, Model model) {
        ...
        List<Cours> cours= salleDeClasse.getCoursList();
        model.addAttribute("salleDeClasse", salleDeClasse);
        model.addAttribute("cours", cours);
        return "détails-salles";
    }
